...
When the target model is passed to the argument `model`, `interpret()` replaces the response variable with the values predicted by the target model. Thus, the fitted MID model can be viewed as *an interpretable model of the target model*.
...
The following plot compares the *permutation feature importance* (PFI) of the target `ranger` model and the term importance (TI) of the MID surrogate model.
...
It is easy to generate *individual conditional expectation* (ICE) plots for the fitted MID surrogate model. In addition, the ICE can be decomposed into the effects of the component functions.

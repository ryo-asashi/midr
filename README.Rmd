...
The **midr** package is designed to provide a model-agnostic method for interpreting black-box machine learning models by creating a globally interpretable surrogate of the target model using a functional decomposition technique called *Maximum Interpretation Decomposition* (MID).

For the theoretical details of MID, see Iwasawa & Matsumori (2025) [Forthcoming], and for the technical details of the package, see Asashiba et al. (2025) [Forthcoming].
...
In the following example, we fit a random forest model of `Boston` dataset included in the ISLR2 package.
...
The main function of the package is `interpret()`, which can be used to create a global surrogate of the random forest model using MID.
...
The graphing functions `ggmid()` and `plot()` can be used to visualize the main effects and the interactions of the variables.
...
The `mid.importance()` function helps to compute and compare the importance of main effects and interactions of the variables in the whole data or for an instance.
...
The `mid.conditional()` function can be used to compute the ICE curves (Goldstein et al. 2015) of the fitted MID model and the breakdown of the ICE curves by main effects and interactions.

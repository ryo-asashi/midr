...
accent <- color.theme("Accent", source = "grDevices")
print(accent)
accent$palette(5)
...
themes[[9L]] <- data.frame(
  pack = "midr",
  name = c("midr", "shap", "grayscale", "bluescale", "HCL"),
  type = c("d", "s", "s", "s", "q")
)
...
for (i in seq_len(nrow(df))) {
  ct <- color.theme(df[i, 2L], source = df[i, 1L])
  text <- sprintf("%s#%s", df[i, 1L], df[i, 2L])
  plot(ct, text = text)
}
```

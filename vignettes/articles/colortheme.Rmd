...
pkgs <- c("khroma", "viridisLite", "RColorBrewer", "gridExtra", "ggplot2", "DT")
for (pkg in pkgs) {
...
accent <- color.theme("Paired", source = "grDevices")
print(accent)
# qualitative color theme "Paired" (package:RColorBrewer)
accent2 <- color.theme("RColorBrewer#Paired")
print(accent2)
```
...
```{r, fig.height = 1.2, fig.width = 6, echo = -1}
par.midr()
# create new color theme using color function
...
grid.arrange(
  ...
  p + scale_fill_theme(color.theme(
    "highlight", kernel.args = list(which = 3)
  ))
)
```

...
pkgs <- c("khroma", "viridisLite", "RColorBrewer", "gridExtra", "ggplot2", "DT", "colorspace")
for (pkg in pkgs) {
...
```{r, fig.height = 0.35, fig.width = 6, echo = c(-1)}
par.midr(mar = c(0, 1, 0, 1))
swatchplot(nightfall$palette(5))
swatchplot(nightfall$ramp(c(0.00, 0.25, 0.50, 0.75, 1.00)))
```
...
```{r, fig.height = 0.35, fig.width = 6, echo = c(-1)}
par.midr(mar = c(0, 1, 0, 1))
swatchplot(viridis$palette(5))
swatchplot(viridis$ramp(c(0.00, 0.25, 0.50, 0.75, 1.00)))
```
...
2.  **Using Suffixes** : for convenience, you can append a suffix directly to the theme's name. `_r` to reverse the theme, `@qual` / `@q` to make the theme qualitative (`@div` / `@d` for diverging, `@seq` / `@s` for sequential).
...
2.  **Using Prefix** : append a prefix to the theme name with the package name and a hash (e.g., `"RColorBrewer#Paired"`).
...
```{r, fig.height = 0.35, fig.width = 6, echo = c(-1)}
par.midr(mar = c(0, 1, 0, 1))
swatchplot(mytheme$palette(5))
swatchplot(mytheme$ramp(c(0.00, 0.25, 0.50, 0.75, 1.00)))
```
...
```{r, fig.height = 0.35, fig.width = 6, echo = c(-1)}
par.midr(mar = c(0, 1, 0, 1))
swatchplot(rainbow$palette(5))
swatchplot(rainbow$ramp(c(0.00, 0.25, 0.50, 0.75, 1.00)))
```
...
grid.arrange(
  ...
  p + scale_color_theme(color.theme(
    "highlight", kernel.args = list(base = "#00000005", which = 3)
  ))
)
```
...
grid.arrange(
  ...
  p + scale_fill_theme(color.theme(
    "highlight", kernel.args = list(accent = "#FF4565", which = 3)
  ))
)
```

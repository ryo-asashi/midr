...
pkgs <- c("mlbench", "gridExtra", "ggplot2", "Metrics", "patchwork", 
...
ice_theme <- color.theme("Zissou 1", "sequential")
...
(importance_plot(mid) / eval_plot(model, mid)) | ice_plot(mid)
```
...
evp <- eval_plot(model, mid, data = test[, -11],
                       s = model$lambda[9])
(importance_plot(mid) / evp) | ice_plot(mid)
```

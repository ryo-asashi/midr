...
#' @param plot.main logical. If TRUE, lines, bars or rectangles representing mid values are not drawn.
...
#' @param m an integer specifying the coarseness of the grid for a "raster" type interaction plot.
...
#'
ggmid <- function(
    x, term, limits = c(NA, NA), plot.main = TRUE, add.intercept = FALSE,
  ...
  if ((len <- length(tags)) == 1L) {
    ...
    df <- x$interactions[[term]]
    df <- stats::na.omit(df)
    if (add.intercept)
      df$mid <- df$mid + x$intercept
    if (include.main.effects)
      df$mid <- df$mid + mid.f(x, tags[1L], df) + mid.f(x, tags[2L], df)
    for (tag in tags) {
    ...
    mpt <- ifelse(add.intercept, x$intercept, 0)
    pl <- ggplot2::ggplot(data = df,
    ...
    if (plot.main) {
      ...
      if (interaction.type == "raster") {
        ...
        colnames(rdf) <- tags
        rdf$mid <- mid.f(x, term, rdf)
        if (add.intercept)
          rdf$mid <- rdf$mid + x$intercept
        if (include.main.effects)
          rdf$mid <- rdf$mid + mid.f(x, tags[1L], rdf) + mid.f(x, tags[2L], rdf)
...
#' @rdname ggmid
#' @param object a mid object to be visualized with \code{ggplot2::autoplot}.

...
mid.plots <- function(
  ...
  if (engine == "ggplot2") {
    plots <- list()
    for (term in terms) {
      pl <- ggmid(x, term, limits = limits, add.intercept = add.intercept, ...)
      plots[[term]] <- pl
    }
    return(plots)
  } else {
    for (term in terms)
      plot.mid(x, term, add.intercept = add.intercept, ...)
  }
}
